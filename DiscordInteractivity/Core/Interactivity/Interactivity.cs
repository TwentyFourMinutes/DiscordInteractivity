...
/// Inherit from this class to get all sort of useful features.
/// </summary>
public class Interactivity<T> : ModuleBase<T>
    where T : SocketCommandContext
{
    /// <summary>
    /// Your <see cref="InteractivityService"/> instance you provided in the dependency injection.
    /// </summary>
    public required InteractivityService InteractivityService { get; init; }
    ...
    protected async Task<IUserMessage> ReplyAndDeleteAsync(
        string? text = null,
        bool isTTS = false,
        Embed? embed = null,
        ...
        await Context
            .Channel.SendAndDeleteMessageAsync(text, isTTS, embed, timeOut, options)
    ...
    /// <param name="options">The options to be used while sending the request.</param>
    /// <exception cref="InvalidOperationException"/>
    protected async Task<IUserMessage> ReplyAndDeleteFileAsync(
        string filePath,
        string? text = null,
        bool isTTS = false,
        Embed? embed = null,
        ...
        await Context
            .Channel.SendAndDeleteFileAsync(filePath, text, isTTS, embed, timeOut, options)
            .ConfigureAwait(false);
    ...
    protected async Task<IUserMessage> ReplyAndDeleteFileAsync(
        ...
        await Context
            .Channel.SendAndDeleteFileAsync(stream, filename, text, isTTS, embed, timeOut, options)
            .ConfigureAwait(false);
    ...
    /// <param name="options">The options to be used while sending the request.</param>
    /// <exception cref="InvalidOperationException"/>
    protected async Task<IUserMessage> ReplyPaginatorAsync(
        Paginator paginator,
        TimeSpan? timeOut = null,
        RequestOptions? options = null
    ) =>
    ...
    /// <exception cref="InvalidOperationException"/>
    protected async Task<WaitingMessageResult> WaitForMessageAsync(
        IUser user,
        bool ignoreCommands = true,
        TimeSpan? timeOut = null
    ) =>
        await Context
            .Channel.WaitForMessageAsync(user, ignoreCommands, timeOut)
            .ConfigureAwait(false);
    ...
    /// <param name="timeOut">The <see cref="TimeSpan"/> it waits for the user.</param>
    /// <exception cref="InvalidOperationException"/>
    protected async Task<WaitingReactionResult> WaitForReactionAsync(
        IUser user,
        TimeSpan? timeOut = null
